...
test('nothing large is committed', () => {
  ...
  //
  // The desktop launcher's icon set is generated by the fleet's gen-launcher
  // and committed, and Tauri's icon.icns carries the 1024 px retina variant
  // inside it — 559 KB on 2026-09-07, the day it landed and this test started
  // failing every deploy for a week. Bounded at 1 MiB like a screenshot: an
  // icon that outgrows that is not an icon.
  const ALLOWANCES = [
    [/^docs\/screenshots\/[^/]+\.png$/, 1024 * 1024],
    [/^docs\/USER-GUIDE\.pdf$/, 4 * 1024 * 1024],
    [/^launcher\/src-tauri\/icons\/[^/]+\.(icns|ico|png)$/, 1024 * 1024],
  ];
